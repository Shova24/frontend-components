...
onMounted(() => {
  ...
  const createArcSVG = (radius) => {
    ...
    svg.style.top = "0"; // Changed from 40dvh to 0
    svg.style.left = "0";
    svg.style.width = "100%";
    ...
    const y1 = window.innerHeight / 2 + Math.sin(startAngle) * radius;
    const x2 = window.innerWidth / 2 + Math.cos(endAngle) * radius;
    const y2 = window.innerHeight / 2 + Math.sin(endAngle) * radius;
  ...
  const positionCards = (progress = 0) => {
    ...
    cards.forEach((card, index) => {
      ...
      const skewX = Math.sin(cardProgress * Math.PI) * 10;
      const skewY = Math.cos(cardProgress * Math.PI) * 10;
  ...
  ScrollTrigger.create({
    ...
    markers: false, // Removed markers as requested
  });
